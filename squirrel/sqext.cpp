...
SQRESULT sq_ext_getfuncinfo(HSQOBJECT obj, SQFunctionInfo *fi)
{
  ...
  {
    SQNativeClosure *c = sq_get_nativeclosure(obj);

    fi->funcid = (SQUserPointer)c->_function;
    fi->name = sq_type(c->_name) == OT_STRING?sq_get_stringval(c->_name):_SC("unknown");
